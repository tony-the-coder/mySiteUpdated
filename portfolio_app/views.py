...
try:
    ...
    PillowImage = None
    logging.warning(
        "Pillow library not installed. Some image functionalities might be limited."
    )
...
def is_office_staff(user):  # For your portfolio, this might just become `user.is_staff`
    ...
    return user.is_active and (
        user.is_staff or user.groups.filter(name="OfficeStaff").exists()
    )
...
def contact_us(request):
    ...
    form = (
        ContactForm()
    )  # You might still pass an empty form for non-JS fallback or initial structure
    context = {
...
def blog_post_detail(request, slug):
    ...
    if post_instance:
        base_query = (
            ...
            .exclude(pk=post_instance.pk)
            .select_related("category")
        )
        if post_instance.category:
            related_posts = base_query.filter(category=post_instance.category).order_by(
                "-published_date"
            )[:3]
        ...
        breadcrumbs.append(
            {
                ...
                "url": reverse(
                    "portfolio_app:blog_category_list",
    ...
    breadcrumbs.append(
        {
            "name": Truncator(post_instance.title).chars(40, truncate="..."),
            "is_active": True,
        }
    )
    context = {
        ...
        "meta_description": (
            post_instance.excerpt
            or Truncator(strip_tags(post_instance.content)).words(25, truncate="...")
        ),
...
def blog_category_list(request, slug):
    ...
    context = {
        "category": category,
        "blog_posts": posts,
        "categories_for_sidebar": all_categories,
        "page_title": f"{category.name} Posts - Tony's Tech Blog",
        "meta_description": category.description
        or f"Explore blog posts by Tony the Coder in the '{category.name}' category.",
        "breadcrumbs": breadcrumbs,
        "is_staff_portal": False,
    }
...
def api_portfolio_projects(request):
    ...
    for p in projects:
        project_data = {
            ...
            "imageUrl": None,
            "categories": [
                {"name": cat.name, "slug": cat.slug} for cat in p.categories.all()
            ],
...
def api_portfolio_categories(request):  # New API view for categories
    ...
    )  # Used in BlogPost category count
    categories = (
        PortfolioCategory.objects.filter(is_active=True)
        .annotate(
            num_active_projects=Count(
                "portfolio_projects", filter=Q(portfolio_projects__is_active=True)
            )
        )
        .filter(num_active_projects__gt=0)
        .order_by("name")
    )
    data = [
...
def staff_dashboard(request):
    active_portfolio_projects_count = PortfolioProject.objects.filter(
        is_active=True
    ).count()
    draft_blog_posts_count = BlogPost.objects.filter(
        ...
        "breadcrumbs": [
            {
                ...
                "url": reverse("portfolio_app:staff_dashboard"),
            },
...
def staff_portfolio_list(request):
    portfolio_items = PortfolioProject.objects.select_related("categories").order_by(
        "order", "-created_at"
    )
    context = {
...
def staff_portfolio_add(request):
        ...
        if form.is_valid():
            ...
            for uploaded_file in new_gallery_images:
                ...
                if ext not in allowed_extensions:
                    messages.error(
                        request,
                        f"Invalid file type: {uploaded_file.name}. Only image files allowed.",
                    )
                    continue
                if PillowImage:
                    try:
                        ...
                        logger.error(
                            f"Pillow could not verify gallery image {uploaded_file.name}: {e}"
                        )
                        messages.error(
                            request,
                            f"File {uploaded_file.name} could not be verified as a valid image.",
                        )
                        continue
            ...
            messages.success(
                request,
                f'Coding Project "{project_instance.title}" created successfully.',
            )
            if images_uploaded_count > 0:
                messages.info(
                    request, f"{images_uploaded_count} new gallery image(s) uploaded."
                )
            return redirect(
                reverse(
                    "portfolio_app:staff_manage_portfolio_images",
                    kwargs={"pk": project_instance.pk},
                )
            )
        else:
    ...
    context = {
        ...
        "breadcrumbs": [
            ...
            {
                "name": "Manage Coding Projects",
                "url": reverse("portfolio_app:staff_portfolio_list"),
            },
...
def staff_portfolio_edit(request, pk):
    ...
    if request.method == "POST":
        form = StaffPortfolioProjectForm(
            request.POST, request.FILES, instance=project_instance
        )
        if form.is_valid():
            ...
            for uploaded_file in new_gallery_images:
                ...
                if PillowImage:
                    try:
                        ...
                        continue
                PortfolioImage.objects.create(
                    portfolio_project=updated_project,
                    image=uploaded_file,
                    caption="",
                    order=0,
                )
            ...
            messages.success(
                request,
                f'Coding Project "{updated_project.title}" updated successfully.',
            )
            if images_uploaded_count > 0:
                messages.info(
                    request, f"{images_uploaded_count} new gallery image(s) uploaded."
    ...
    context = {
        ...
        "breadcrumbs": [
            ...
            {
                "name": f'Edit: {Truncator(project_instance.title).chars(30, truncate="...")}',
                "is_active": True,
            },
        ],
        "is_staff_portal": True,
...
def staff_manage_portfolio_images(request, pk):
    ...
    queryset = PortfolioImage.objects.filter(portfolio_project=project).order_by(
        "order", "id"
    )
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            for obj in formset.deleted_objects:
                if (
                    obj.image
                    and hasattr(obj.image, "path")
                    and os.path.isfile(obj.image.path)
                ):
                    try:
            ...
            if featured_image_id_str:
                try:
                    ...
                    selected_gallery_image = PortfolioImage.objects.get(
                        id=featured_image_id, portfolio_project=project
                    )
                    if selected_gallery_image.image:
                        if (
                            not project.featured_image
                            or project.featured_image.name
                            != selected_gallery_image.image.name
                        ):
                            ...
                            img_file_content = ContentFile(
                                selected_gallery_image.image.read()
                            )
                            file_name = os.path.basename(
                                selected_gallery_image.image.name
                            )
                            project.featured_image.save(
                                file_name, img_file_content, save=False
                            )  # Don't save project yet
                            messages.success(
                                request,
                    ...
                    messages.error(
                        request,
                        f"An error occurred while setting the featured image: {e}",
                    )

            if formset.has_changed() or formset.deleted_objects:
                messages.success(request, "Gallery image details updated successfully!")
            elif (
                not featured_image_id_str
            ):  # Only show if no other message was more relevant
    ...
    context = {
        ...
        "current_featured_image_name": (
            os.path.basename(project.featured_image.name)
        ...
        "breadcrumbs": [
            ...
            },
            {
                ...
                "url": reverse(
                    "portfolio_app:staff_portfolio_edit", kwargs={"pk": project.pk}
                ),
            },
...
def portfolio_project_detail_staff(request, pk):
    project = get_object_or_404(
        PortfolioProject.objects.select_related("categories"), pk=pk
    )
    context = {
        "project": project,
        ...
        "breadcrumbs": [
            ...
            {
                "name": Truncator(project.title).chars(30, truncate="..."),
                "is_active": True,
            },
        ],
        "is_staff_portal": True,
    ...
    return render(
        request, "portfolio_app/staff/portfolio_project_detail_staff.html", context
    )
...
def staff_portfolio_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f'Coding Project "{project_title}" and its images deleted.'
        )
    ...
    context = {
        ...
        "breadcrumbs": [
            ...
            {
                "name": f'Delete: {Truncator(project_title).chars(30, truncate="...")}',
                "is_active": True,
            },
        ],
        "is_staff_portal": True,
    ...
    return render(
        request, "portfolio_app/staff/portfolio_project_confirm_delete.html", context
    )
